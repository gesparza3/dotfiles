...
require("lazy").setup({
	-- Statusline
	{
		...
		config = function()
			require("lualine").setup({
				options = {
					theme = "auto",
					globalstatus = true, -- uses laststatus=3
					icons_enabled = true,
					section_separators = { left = "", right = "" },
					component_separators = { left = "│", right = "│" },
					disabled_filetypes = { "NvimTree", "lazy", "Trouble" },
				},
				-- tweak sections if you want
				sections = {
					lualine_a = { "mode" },
	...
	},
	{
		...
		opts = {
			-- Only format the buffer you're actively saving (normal files, not NvimTree/terminal/etc.)
			format_on_save = function(bufnr)
				local bo = vim.bo[bufnr]
				-- skip special/readonly/unmodifiable buffers
				if bo.buftype ~= "" or bo.readonly or not bo.modifiable then
					return
				end
				-- only format the current (active) buffer
